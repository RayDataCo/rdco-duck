    ...
    create  table
      "dbt"."main"."mrr_mrr__dbt_tmp"
    as (
      with unioned as (

    select * from "dbt"."main"."mrr_customer_revenue_by_month"

    union all

    select * from "dbt"."main"."mrr_customer_churn_month"
...
md5(cast(coalesce(cast(date_month as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(customer_id as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as id,
